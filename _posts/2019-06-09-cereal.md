...
1.	Some cereal manufacturers are healthier than others
    ...
    3. Nabisco has the lowest fat levels, with American Home Food Products as the next lowest brand. <br>
2.	Grocery stores frequently place the unhealthiest cereals at eye level
    ...
    3. Shelf 2 has the lowest potassium and the lowest fiber. Shelf 3 has the highest potassium levels and the highest fiber amounts. Shelf 1 is in between. <br>
3.	What impacts the rating the cereal receives?
    ...
    3. The variables that we included in the final model had an Adjusted R Square of 93%, meaning those variables explained 93% of the rating a cereal receives. <br>
4.	After creating the final model, I wanted to test the equation we came up with to predict cereal rating. Sadly, I had not separated out some of the dataset at the beginning of the analysis so I could not do that. Next time, I will split the data into 80/20, 80% to train the model, and 20% to test the model’s accuracy. <br>
