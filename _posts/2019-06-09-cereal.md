...
I used the ggplot2 package for the graph. Follow this link if you want to see the code.
...
Let’s look at average fat & sugars levels in a table format across manufacturers to see if it confirms the conclusions we drew from the visual display.

This revealed that Quaker has the highest average fat levels (wasn’t quite decipherable in the chart).
...
Relationship between shelf level and potassium(num)/fiber(num)/vitamins(ftr)/protein(ftr) – Are grocery stores putting the least healthy or most healthy at eye level? We are making the assumption that Shelf 2 is at eye level.
...
This quickly shows us Shelf 2 has the lowest potassium and the lowest fiber. Shelf 3 has the highest potassium levels and the highest fiber amounts. Shelf 1 is in between. In this case a table is more useful than the above graph.
...
Now that we have a good feel for the data, let’s look at how the variables impact rating of cereal. We’ll run a linear regression model, this shows us two things:

1. Do the independent variables do a good job of predicting the dependent variables
2. Which variables have a greater impact on the dependent variable?
  1. To put this in perspective, do the nutrition facts (independent variables) explain the rating (dependent variable) the cereal received?
...
Before we analyze the results, I want to explain something about the transformed variables. When the protein variable was transformed from one numeric variable into 6 factors, we see each of those broken out in the model. This is helpful to us because for example, when protein level is very high (cerealFile$ftrprotein6) it has the Estimate 8.6, whereas when the protein level is lower (protein2 & protein3), the Estimate is 4.13 & 4.56. Thus, a higher protein level has a strong positive impact on the rating.
...
This is the equation for the model: y = 58.05 + 2.32x1(fiber) – 1.44x2(sugars) + 4.65x3(protein2) + 6.1x4(protein3) + 11.75x5(protein4) – 5.77x6(protein5) + 9.48x7(protein6)  - 5.15x8(fat1) – 9.17x9(fat2) -12.05x10(fat3) – 23.28x11(fat5) – 11.6x12(vitamins25) – 16.41x13(vitamins100)
